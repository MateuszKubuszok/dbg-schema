...
object ShowSchema extends TraverseSchema {
  ...
  def onNothing(using Context): Out[Nothing] = _ => ctx.indent.append("???")
  def onNull(using Context): Out[Null]       = _ => ctx.indent.append("null")
  def onUnit(using Context): Out[Unit]       = _ => ctx.indent.append("()")
  def onByte(using Context): Out[Byte]       = a => ctx.indent.append(a.toString + ".toByte")
  def onBoolean(using Context): Out[Boolean] = a => ctx.indent.append(a.toString)
  def onShort(using Context): Out[Short]     = a => ctx.indent.append(a.toString + ".toShort")
  def onInt(using Context): Out[Int]         = a => ctx.indent.append(a.toString)
  def onLong(using Context): Out[Long]       = a => ctx.indent.append(a.toString + "L")
  def onFloat(using Context): Out[Float]     = a => ctx.indent.append(a.toString + "f")
  def onDouble(using Context): Out[Double]   = a => ctx.indent.append(a.toString)
  def onChar(using Context): Out[Char]       = a => ctx.indent.append("'" + a.toString + "'")
  def onString(using Context): Out[String]   = a => ctx.indent.append("\"" + a.toString + "\"")
